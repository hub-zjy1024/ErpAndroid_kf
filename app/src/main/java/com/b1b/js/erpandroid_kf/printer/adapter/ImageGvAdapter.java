package com.b1b.js.erpandroid_kf.printer.adapter;
...
public class ImageGvAdapter extends MyBaseAdapter<PrinterItem> {
    ...
    protected void initHolder(final PrinterItem currentData, MyBasedHolder baseHolder) {
        ...
        Bitmap bitmap = MyImageUtls.bitmapCache.get(currentData.getFile().getAbsolutePath());
        if (bitmap == null) {
            new Thread(){
                @Override
                public void run() {
                    ...
                    MyImageUtls.bitmapCache.put(currentData.getFile().getAbsolutePath(), newBitmap);
