...
public class FtpUpFile {
    ...
    private void connectAndLogin(String name, String password, String ftpUrl, int port) throws IOException {
        if (port == 0) {
    ...
    public boolean upload(File file, String remotePathName,
        ...
        connectAndLogin(name, password, ftpUrl, port);
        changeDirectory(remotePathName);
        boolean isSuccess = uploadFile(file, remotePathName, remoteName);
        backToRootDirectory();
    ...
    public boolean upload(InputStream inputStream, String remotePathName,
        ...
        connectAndLogin(name, password, ftpUrl, port);
        changeDirectory(remotePathName);
        boolean isSuccess = uploadFile(inputStream, remotePathName, remoteName);
        backToRootDirectory();
        ftpClient.disconnect();
        return isSuccess;
    }
    ...
    private boolean uploadFile(File file, String remotePathName, String remoteName) throws IOException, RemoteDeleteException {
        if (file == null) {
            return false;
        }
    ...
    private boolean uploadFile(InputStream inputStream, String remotePathName, String
