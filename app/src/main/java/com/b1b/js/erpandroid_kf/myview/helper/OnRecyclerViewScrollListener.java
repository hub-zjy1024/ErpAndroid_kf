package com.b1b.js.erpandroid_kf.myview.helper;
...
public abstract class OnRecyclerViewScrollListener extends RecyclerView.OnScrollListener implements OnBottomListener {
    static class LAYOUT_MANAGER_TYPE {
   ...
   /* public enum LAYOUT_MANAGER_TYPE {
    ...
    /*用于判断滑动方向，dy等于0，表示RecyclerView没有滑动，比如条目没充满RecyclerView的话，手势不管哪个方向滑都是0；dy大于0，RecyclerView向上滑动；dy小于0
    ，RecyclerView向下滑动*/
    private int mDy;
    ...
    public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
        ...
        switch (mLayoutManagerType) {
            case LAYOUT_MANAGER_TYPE.LINEAR:
                mLastVisibleItemPosition =
                        ((LinearLayoutManager) layoutManager).findLastVisibleItemPosition();
                break;
            case LAYOUT_MANAGER_TYPE.GRID:
                mLastVisibleItemPosition = ((GridLayoutManager) layoutManager).findLastVisibleItemPosition();
                break;
            case LAYOUT_MANAGER_TYPE.STAGGERED_GRID:
    ...
    @Override
    abstract public void onBottom(View v);
